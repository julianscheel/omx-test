...
int main(int argc, char** argv) {
	...
	char *componentName = "OMX.broadcom.image_fx";
	...
	for (i = 0; i < nPorts; i++) {
		...
		if (sPortDef.eDomain != OMX_PortDomainImage) {
			fprintf(stderr, "Port %d is not an image port\n", sPortDef.nPortIndex);
			exit(1);
		}
		...
		if (sPortDef.format.image.eColorFormat == OMX_COLOR_FormatYUV420PackedPlanar)
		...
		sPortDef.format.image.nFrameWidth = 1920;
		sPortDef.format.image.nFrameHeight = 1080;
		sPortDef.format.image.nStride =
			ALIGN(sPortDef.format.image.nFrameWidth, 32);
		sPortDef.format.image.nSliceHeight =
			ALIGN(sPortDef.format.image.nFrameHeight, 16);
